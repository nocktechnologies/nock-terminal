...
Nock Terminal is an Electron desktop app with a React renderer. The renderer owns the cockpit UI; the Electron main process owns all privileged work: PTYs, filesystem access, settings, process discovery, network clients, and OS integrations.

## Process Model

- `src/main.jsx` mounts the React app.
- `src/App.jsx` owns top-level renderer state for the active view, terminal tabs, session list, port list, process status, chat panel, and active project.
- `electron/main.js` creates the frameless window, tray, global shortcuts, service instances, and IPC handlers.
- `electron/preload.js` exposes a narrow `window.nockTerminal` API through `contextBridge`.
- The renderer never imports Node APIs directly. It calls preload methods, which route to `ipcMain` handlers and main-process services.

## Main Surfaces

- **Dashboard** - `Dashboard`, `ProjectCard`, and `Sidebar` show discovered Claude Code sessions, git repos, status counts, ports, project context, prompts, and session history.
...
- `TerminalManager` wraps `node-pty`, chooses a platform shell, spawns interactive/login shells where needed, relays terminal data, resizes PTYs, chunks large writes on Windows, and destroys processes.
- `SessionDiscovery` reads Claude Code transcripts from `~/.claude/projects`, scans configured dev roots for git repos, merges them by path, and annotates branch/dirty/status data.
...
- `ProcessDetector` observes terminal processes and reports Claude activity status to the renderer.
...
3. `main.js` grants those project paths to `FileService` and revalidates `FileWatcher`.
4. Dashboard and sidebar render sessions, project status, file trees, context checks, and dashboard cards from the returned data.
...
4. xterm sends user input back through `terminal:write`; resize events use `terminal:resize`.
5. Session history records output only when `autoCaptureSessions` is enabled.

### Files And Git

1. The sidebar chooses an active project path from the active tab cwd or first discovered session.
...
- Every 60 seconds, `heartbeat()` sends `PATCH /api/terminal/sessions/{id}/` with active project and Claude session fields. The current caller sends `activeProjectCount: 0` and `activeClaudeSessionIds: []`.
