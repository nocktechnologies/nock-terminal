...
Claude-specific note: this app still has first-class Claude Code transcript discovery and launch flows. It also discovers local agent folders from existing `config.json` files and can route Codex/DeepSeek dispatch-agent tasks through Mira or direct CRM dispatcher scripts. True reconnect/attach and non-Claude transcript discovery are still future adapter work, so keep product copy honest while the broader agent-agnostic layer continues.
