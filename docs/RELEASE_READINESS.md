...
- Terminal usability validation is complete for the installed arm64 development artifact: a disposable trusted CRM attach confirmed selection copying, and Mira's remote resident session confirmed wheel scrollback plus Claude OAuth-prompt OSC 52 output. A rebuilt local-terminal probe confirmed that unsolicited OSC 52 output is denied and expected output requires a renderer-owned, full-value/source confirmation before copying. Unsafe invisible or control characters are rejected, and OSC 52 values are redacted from optional session capture. Capture is established only for sessions opened while auto-capture is enabled and stops permanently for a session when disabled, keeping uncaptured output off the main-process parsing path. The OAuth URL was not opened and authentication was cancelled. Signed release-candidate artifacts still require the platform evidence recorded below.
