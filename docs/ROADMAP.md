...
Terminal wheel routing now follows xterm's live mouse-tracking mode, so local and manually entered remote tmux attaches receive wheel input without launch-path metadata. Resident tmux servers enable mouse handling and OSC 52 forwarding; after a plain `c` gesture in the active focused terminal, the renderer shows the complete bounded value and source tab and requires an explicit Copy confirmation before writing the clipboard. Control, bidi, invisible-format, and line-separator characters are rejected, and OSC 52 values are statefully redacted from optional session capture across PTY chunk boundaries. Terminal output also supports macOS Cmd+C, Ctrl+Shift+C, a stable right-click copy action, and Option-assisted selection for mouse-reporting TUIs. The installed arm64 app was validated against Mira's remote resident session: wheel input moved backward through tmux history and Claude's OAuth prompt emitted the expected OSC 52 URL without opening it or completing authentication. A rebuilt local-terminal probe then validated both the renderer confirmation path and denial of unsolicited OSC 52 output.
...
Phase H is the post-Phase-F push from private-alpha dogfoodable to public-beta credible. It has two tracks that should not be blurred:

- **Release readiness**: signed artifact smoke, real credentials, update-channel decision, crash/error reporting, support path, beta feedback, and rollback evidence.
- **Product truth**: dispatch completion tracking, adapter transcript discovery, and real resume/attach semantics without overclaiming support for Codex/Gemini until those flows exist.

See `docs/PHASE_H_EXECUTION_PLAN.md` for the ordered slices, dependencies, acceptance criteria, verification, and parallelization guidance.
...
This is not the end-state orchestration system. It is the first cohesive cockpit interaction that makes the product feel agent-agnostic without overstating reconnect support.
