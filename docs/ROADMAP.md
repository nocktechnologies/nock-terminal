...
- Nock `7628` / Phase H H4a closed in PR #53 with the local dispatch status reducer and history normalization foundation. It deliberately stops short of live completion polling until NockCC exposes a confirmed live-message read or subscription API.
...
- **Product truth**: dispatch completion tracking, adapter transcript discovery, and real resume/attach semantics without overclaiming support for Codex/Gemini until those adapters exist.
