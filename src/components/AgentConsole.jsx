...
import {
  Bot, CheckCircle2, CirclePause, CirclePlay, Compass, KeyRound, Loader2,
  Play, Plus, RefreshCw, RotateCcw, Send, ShieldCheck, Square, Terminal,
  TriangleAlert, Wrench, Pencil, X,
} from 'lucide-react';
import AgentCreateWizard from './AgentCreateWizard';
import {
  buildAgentInventory, describeAgentError, normalizePrerequisites, unwrapManagedResponse,
} from '../utils/agentConsole.mjs';

const LIFECYCLE_TONES = {
  running: 'bg-nock-green', idle: 'bg-nock-green', starting: 'bg-nock-accent-cyan',
  paused: 'bg-nock-accent-amber', needs_auth: 'bg-nock-accent-amber',
  invalid: 'bg-nock-red', terminal_failed: 'bg-nock-red', stale: 'bg-nock-yellow',
};

export default function AgentConsole({ sessions = [], onOpenSession, onOpenAuth, onRefreshSessions }) {
  ...
  const refresh = useCallback(async ({ includeSessions = false } = {}) => {
    if (!api) {
      setLoading(false);
      setNotice({ tone: 'error', text: 'Managed agent controls are unavailable in this build.' });
      return;
    }
    setRefreshing(true);
    try {
      const [prerequisiteResponse, listResponse] = await Promise.all([api.prerequisites(), api.list()]);
      const nextPrerequisites = unwrapManagedResponse(prerequisiteResponse, 'prerequisites');
      const nextManaged = unwrapManagedResponse(listResponse, 'agents');
      setPrerequisites(normalizePrerequisites(nextPrerequisites));
      setManagedRows(Array.isArray(nextManaged) ? nextManaged : []);
      if (includeSessions) await onRefreshSessions?.();
    } catch (error) {
      const message = describeAgentError(error);
      setPrerequisites([{ id: 'agent-console', label: 'Agent Console', available: false, reason: message }]);
      setNotice({ tone: 'error', text: message });
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [api, onRefreshSessions]);

  useEffect(() => { refresh(); }, [refresh]);

  const inventory = useMemo(() => buildAgentInventory(managedRows, sessions), [managedRows, sessions]);
  ...
  const createBlockedReason = !api
    ? 'Managed agent controls are unavailable in this build.'
    : (loading
  ...
  const createResident = useCallback((draft) => run({ key: 'create', displayName: draft.displayName }, 'create', () => api.create(draft)), [api, run]);
  const updateResident = useCallback((draft) => run(editingAgent, 'edit', () => api.update(editingAgent.id, draft)), [api, editingAgent, run]);
  const openAuth = useCallback(async (agent) => {
    const result = await run(agent, 'authenticate', () => api.authLaunch(agent.id));
    if (result) onOpenAuth?.(result);
  }, [api, onOpenAuth, run]);
  const control = useCallback((agent, action, params) => run(agent, action, () => api.control(agent.id, action, params)), [api, run]);
  const supervise = useCallback((agent, action) => run(agent, action, () => api.supervise(agent.id, action)), [api, run]);
  const validate = useCallback((agent) => run(agent, 'validate', () => api.validate(agent.id)), [api, run]);

  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-nock-bg">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-nock-border px-6 py-4">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <Bot className="h-4 w-4 text-nock-accent-cyan" aria-hidden="true" />
            <span className="font-mono text-[10px] uppercase tracking-widest text-nock-accent-cyan">// Agent Console</span>
          </div>
          <p className="mt-1 font-mono text-xs text-nock-text-dim">{inventory.length} agent{inventory.length === 1 ? '' : 's'} in inventory</p>
        </div>
        <div className="flex items-center gap-2">
          <IconButton label="Refresh agents" onClick={() => refresh({ includeSessions: true })} disabled={refreshing}>
            <RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin' : ''}`} aria-hidden="true" />
          </IconButton>
          <button type="button" onClick={() => setShowCreate(true)} disabled={Boolean(createBlockedReason)} title={createBlockedReason || 'Create resident'} className="inline-flex items-center gap-2 rounded bg-nock-accent-blue px-3 py-2 font-mono text-[10px] font-semibold uppercase tracking-wider text-white transition-colors hover:bg-nock-accent-blue/80 disabled:cursor-not-allowed disabled:opacity-40">
            <Plus className="h-3.5 w-3.5" aria-hidden="true" /> Create Resident
          </button>
        </div>
      </header>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(290px,0.9fr)_minmax(0,1.7fr)]">
        <aside className="min-h-0 overflow-y-auto border-b border-nock-border lg:border-b-0 lg:border-r">
          <div className="sticky top-0 z-10 flex items-center justify-between border-b border-nock-border bg-nock-bg px-4 py-2.5">
            <span className="font-mono text-[9px] uppercase tracking-widest text-nock-text-muted">Inventory</span>
            <span className="font-mono text-[9px] tabular-nums text-nock-text-muted">{inventory.length}</span>
          </div>
          {loading ? <LoadingLine label="Loading managed agents" /> : inventory.length === 0 ? <EmptyInventory /> : (
            <div className="p-2">
              {inventory.map((agent) => <InventoryRow key={agent.key} agent={agent} selected={agent.key === selected?.key} onClick={() => setSelectedKey(agent.key)} />)}
            </div>
          )}
        </aside>
        ...
        <main className="min-h-0 overflow-y-auto">
          {selected ? (
            <AgentInspector
              ...
              onAttach={() => onOpenSession?.(selected.session)}
              onEdit={() => setEditingAgent(selected)}
              onAuthenticate={() => openAuth(selected)}
      ...
      <AgentCreateWizard open={showCreate} onClose={() => setShowCreate(false)} onCreate={createResident} blockedReason={createBlockedReason} />
      <AgentCreateWizard
  ...
  return (
    <button type="button" onClick={onClick} className={`mb-1 w-full rounded border px-3 py-2.5 text-left transition-colors ${selected ? 'border-nock-accent-blue/60 bg-nock-card-hover' : 'border-transparent hover:border-nock-border hover:bg-nock-card/70'}`}>
      ...
      <div className="mt-1.5 flex min-w-0 items-center gap-1.5 font-mono text-[9px] uppercase tracking-wider">
        <span className={agent.ownership === 'managed' ? 'text-nock-accent-cyan' : 'text-nock-text-muted'}>{agent.ownership}</span>
        <span className="text-nock-text-muted">/</span>
        <span className="truncate text-nock-text-dim">{agent.lifecycleLabel}</span>
        <span className="ml-auto shrink-0 text-nock-text-muted">{agent.locationLabel}</span>
      </div>
...
function AgentInspector({ agent, activeAction, steerText, onSteerText, onAttach, onEdit, onAuthenticate, onValidate, onSupervise, onControl }) {
  ...
  return (
    <div className="py-5 pl-5 pr-20 lg:pl-7 lg:pr-20">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-nock-border pb-5">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="font-display text-xl font-semibold text-nock-text">{agent.displayName}</h1>
            <OwnershipBadge ownership={agent.ownership} />
        ...
        <span className={`rounded border px-2 py-1 font-mono text-[9px] uppercase tracking-wider ${agent.local ? 'border-nock-green/35 text-nock-green' : 'border-nock-accent-amber/35 text-nock-accent-amber'}`}>{agent.locationLabel}</span>
      </div>
      ...
      <section className="border-b border-nock-border py-5">
        ...
        {agent.ownership === 'managed' ? (
          <div className="mt-3 flex flex-wrap gap-2">
            {commands.map(([action, label, Icon, onClick]) => (
              <CommandButton
                key={action}
                visible={agent.capabilities[action]}
                busy={busy(action)}
                disabled={anyBusy || (action === 'attach' && !agent.session)}
                label={label}
                icon={Icon}
                onClick={onClick}
              />
          ...
          <div className="mt-3 flex items-center gap-3">
            <span className="font-mono text-[10px] uppercase tracking-wider text-nock-text-muted">Read only</span>
            <CommandButton visible={agent.capabilities.attach} disabled={!agent.session} label="Attach" icon={Terminal} onClick={onAttach} />
          </div>
        )}
      </section>

      {agent.ownership === 'managed' && agent.capabilities.steer && (
        <section className="border-b border-nock-border py-5">
          ...
          <div className="mt-3 flex gap-2">
            <input
              ...
              disabled={anyBusy}
              className="flex-1 rounded border border-nock-border bg-nock-card/70 px-2.5 py-2 font-mono text-[11px] text-nock-text outline-none transition-colors placeholder:text-nock-text-muted focus:border-nock-accent-blue/60 disabled:cursor-not-allowed disabled:opacity-60"
              placeholder="Send a directive"
              aria-label={`Steer ${agent.displayName}`}
            />
            <CommandButton visible disabled={anyBusy || !steerText.trim()} busy={busy('steer')} label="Send" icon={Send} onClick={invokeSteer} />
          </div>
        </section>
      )}
      ...
      <section className="border-b border-nock-border py-5">
        ...
        <dl className="mt-3 grid gap-x-6 gap-y-0 md:grid-cols-2">
      ...
      {agent.ownership === 'managed' && (
        <section className="py-5">
          <SectionTitle icon={ShieldCheck}>Advanced permissions</SectionTitle>
...
function CommandButton({ visible = true, busy = false, disabled = false, label, icon: Icon, onClick }) {
  if (!visible) return null;
  const invoke = () => { Promise.resolve(onClick?.()).catch(() => {}); };
  return (
    <button
      type="button"
      onClick={invoke}
...
function IconButton({ label, children, ...props }) {
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="inline-flex h-8 w-8 items-center justify-center rounded border border-nock-border text-nock-text-dim transition-colors hover:border-nock-border-bright hover:text-nock-text disabled:cursor-not-allowed disabled:opacity-40"
      {...props}
    >
      {children}
    </button>
...
function OwnershipBadge({ ownership }) {
  const tone = ownership === 'managed'
    ? 'border-nock-accent-cyan/40 text-nock-accent-cyan'
    : 'border-nock-border text-nock-text-muted';
  return <span className={`rounded border px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wider ${tone}`}>{ownership}</span>;
}
...
function SectionTitle({ icon: Icon, children }) {
  return (
    <h2 className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-nock-text-dim">
...
function Info({ label, value, mono = false, multiline = false }) {
  const valueClass = `${mono ? 'font-mono text-[10px]' : ''} ${multiline ? 'max-h-28 overflow-y-auto' : 'truncate'}`;
  return (
    <div className="border-t border-nock-border/70 py-2.5">
      ...
      <dd className={`mt-1 whitespace-pre-wrap break-words text-xs text-nock-text ${valueClass}`} title={String(value)}>{value}</dd>
    </div>
  );
}
...
function EmptyInventory() {
  return <div className="px-4 py-6 font-mono text-[10px] text-nock-text-muted">No discovered or managed agents.</div>;
}

function EmptyInspector() {
  return <div className="flex h-full items-center justify-center px-6 font-mono text-[10px] text-nock-text-muted">No agent selected</div>;
}
