  ...
  if (isAgentFolder) {
    if (session?.launch?.mode === 'dispatch') {
      ...
      return {
        ...
        mode: 'dispatch',
        action: trimString(session?.launch?.action) || 'dispatch',
        actionLabel: trimString(session?.launch?.actionLabel) || 'Dispatch',
        capability: trimString(session?.launch?.capability) || 'dispatch-request',
